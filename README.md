...
- **My original reasoning logic had a real bug I only found by actually testing it.** I originally wrote it so ANY `without_mask` detection would immediately override everything else in the frame, no matter how low its confidence was. When I finally ran the agent on a real photo with the trained model, it flagged a photo of two people wearing masks as NON_COMPLIANT — turns out the model had misread a badge on someone's shirt as an unmasked face, at only 0.55 confidence, way lower than the 0.90 and 0.87 confidence on the two actual correct mask detections. That one low-confidence mistake was enough to override two much more confident correct detections. I had to go back and add a higher confidence bar specifically for violation calls, since a compliance agent flagging someone incorrectly is a bigger deal than it just missing something. This is the kind of bug you only catch by actually running your code on real data, not by just reading it over.
