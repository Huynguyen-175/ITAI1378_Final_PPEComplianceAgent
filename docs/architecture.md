...
```
 ...
 input → preprocessing              status banner). Batch           rules (R1-R4), each
...
| 4. Reasoning | `agents/reasoning.py :: decide_compliance()` | Explicit rule-based logic. Priority order: `without_mask` → NON_COMPLIANT (R1) → `incorrectly_worn_mask` → NON_COMPLIANT (R2) → `with_mask` only → COMPLIANT (R3) → nothing relevant detected → NO_DETECTION, abstain (R4). Every `Decision` records `rule_fired` and a plain-English `explanation`. |
